...
class BranchList1 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_branch_list1)

        list = findViewById(R.id.listView_list1)
        var adapter = ArrayAdapter(this, android.R.layout.simple_list_item_1, branch)
        list.adapter = adapter

        list.setOnItemClickListener { _, _, i, _ ->
            var str = list.adapter.getItem(i).toString()
            val intent = Intent(applicationContext, MainActivity::class.java)
            intent.putExtra("namme", str)
